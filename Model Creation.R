...
comparison %>% summarise(correct_in_top_64 = mean(num/64), not_in_top_64 = mean(na/64), exactly_correct = mean(same/64), avg_diff = mean(diff))


# To find reference categories
filtered_data[!filtered_data$home_team %in% gsub("as\\.factor\\(team\\)", "", rownames(as.data.frame(summary(bt_model)$coef)))[3:330],]
filtered_data[!filtered_data$home_conf %in% gsub("as\\.factor\\(conf\\)", "", rownames(as.data.frame(summary(bt_model)$coef)))[3:330],]
